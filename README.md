...
4. Writes/updates a `physioconvert_map.tsv` at the `--output` root, one row per validated physio file. The BIDS entity columns (`participant_id`, `session_id`, `datatype`, `task`, `acquisition`, `run`) are best-guesses parsed from the input file's basename, split on underscores:
