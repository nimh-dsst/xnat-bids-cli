...
**Fill in the blank entity columns in `physio_map.tsv` and re-run** to place the `UNKNOWN_NAME` files under their proper BIDS path — user edits are preserved across runs, and once a file is placed correctly its earlier `tmp_phys2bids/` copy is deleted. To apply such edits **without** re-running the (slow) `phys2bids` conversion, re-run with `-m/--maps`: it moves/renames each file's existing outputs to match its edited entities instead (see below).
...
| `-m`, `--maps` | *Optional.* Skip the `phys2bids` conversion and instead **apply your `physio_map.tsv` edits** to the already-converted files: each file's existing `_physio.tsv.gz`/`.json` output(s) are **moved/renamed** to the BIDS path implied by its (possibly edited) entities — using the same naming, collision/`run-` numbering, `recording-<freq>Hz` labels, and `tmp_phys2bids/` ↔ proper-path transitions as a full run, but without re-running `phys2bids`. `physio_map.tsv`/`physio_qc.tsv` and their `.json` data dictionaries are regenerated; emptied source directories are pruned. Each file is still re-read to recompute its metrics, so `phys2bids` is needed to read the files. Files with no prior output (e.g. an earlier `CONVERT_ERROR`) cannot be placed in this mode — re-run without `-m/--maps` to convert them. |
