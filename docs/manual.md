...
- **`-1` (single-experiment) mode:** Pass `--rename-subject` and/or `--rename-experiment` on the command line instead — there's no CSV to edit.
...
XNAT itself is always queried using the original `SUBJECT_LABEL`/`EXPERIMENT_LABEL` (or, in `-1` mode, the original `SUBJECT`/`EXPERIMENT` arguments); only the on-disk directory name (and, with `-a/--archive`, the archive filename and, with `-l/--log`, the log's `SUBJECT`/`EXPERIMENT` columns) reflects the rename.
...
- In `-1` mode, an invalid `--rename-subject`/`--rename-experiment` value exits with an error before downloading.

`--rename-subject`/`--rename-experiment` only apply to `-1` mode; supplying either alongside `--csv` is an error.
